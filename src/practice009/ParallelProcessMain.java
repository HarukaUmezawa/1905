...
public class ParallelProcessMain {
	...
	public static void main (String args[]){
		...
		try {
			...
			for(int i=0 ; i<p ; i++){
				resultList.add(ThreadPool.submit(new ParallelProcessSub1(proc.get(i))));
				resultList.add(ThreadPool.submit(new ParallelProcessSub2(proc.get(i))));
				resultList.add(ThreadPool.submit(new ParallelProcessSub3(proc.get(i))));
